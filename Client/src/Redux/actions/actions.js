import {RESET_PAGE,RESET_CHARACTERS,ADD_FAV,REMOVE_FAV,ADD_CHAR,REMOVE_CHAR,
    FILTER,RESET,FILTER_A_Z,PREV,NEXT,CREATE_CHAR,SEARCH_CHAR} from "./types.js";
    ...
    return async function (dispatch) {
        try{
            const { data } = await axios.post(
                `http://localhost:3001/rickandmorty/favourites`,
                char
            );
            return dispatch({
                type: ADD_FAV,
    ...
    return async function (dispatch) {
        try{
            const { data } = await axios.delete(
                `http://localhost:3001/rickandmorty/favourites/${id}`
            );
            return dispatch({
                type:REMOVE_FAV,
                payload: data,
